...
from agvsimulator_02 import *



def build_mlp():
    model = Sequential()
    model.add(Dense(40, activation='relu', input_shape=(410,)))
    model.add(Dense(256))
...
while True:
    ...
    while not terminal:
        ...
        next_state, reward, terminal, _ = env.step([int(i) for i in ('0000000'+format(action,'b'))[-8:]])
...
for episode in range(n_episodes):
    ...
    while not terminal:
        ...
        #reward = np.sign(reward)
        total_reward += reward
        #print(episode,step,total_reward,reward)
        transition = {
            'state': state,
            'next_state': next_state,
            'reward': reward,
