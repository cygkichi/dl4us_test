...
def build_mlp():
    model = Sequential()
    model.add(Dense(100, activation='relu', input_shape=(460,)))
...
def test():
    ...
    while not terminal:
        ...
        total_q_argmax.append(np.argmax(q))
        action = np.argmax(q)
        #print(env.state, action)
        next_state, reward, terminal, _ = env.step([int(i) for i in ('00000000'+format(action,'b'))[-n_branch:]], mode='test')
        next_state = np.clip(next_state.flatten(),0,1)
        #next_state = next_state.flatten()
        total_reward += reward
        state = next_state
        ...
        if step > 1000:
            terminal = 1
    print('test  total_reward: ',total_reward,' total_step: ',step)
    print('test  action',np.histogram(total_q_argmax, bins=n_actions)[0])
